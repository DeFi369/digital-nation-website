(function () {
  ...
  function renderStats() {
    ...
    fetch('assets/data/environment.json')
      ...
      .catch(function () {
        container.innerHTML = '<div class="activity-empty">Environment summary is temporarily unavailable.</div>';
      });
  }
  ...
  function renderPillars() {
    const container = document.getElementById('environment-pillars');
    if (!container) return;
    fetch('assets/data/environment.json')
      .then(function (res) { return res.json(); })
      .then(function (data) {
  ...
  /* ---- policy / programs / standards / reports ---- */
  function renderPolicy() {
    loadSection('environment-policy-feed', 'policy', 'Environmental policy');
  }

  function renderPrograms() {
    loadSection('environment-programs-feed', 'programs', 'Environment programs');
  }

  function renderStandards() {
    loadSection('environment-standards-feed', 'standards', 'Green protocol standards');
  }

  function renderReports() {
    loadSection('environment-reports-feed', 'reports', 'Environment governance and audit reports');
  }
  ...
  function initFilters() {
    var searchField = document.getElementById('environment-search');
    var scopeField = document.getElementById('environment-scope');
    ...
    function applyFilter() {
      ...
      var items = [];
      if (scope === 'all' || scope === 'policy') {
        (cachedData.entries && cachedData.entries.policy || []).forEach(function (entry) {
          items.push(entry);
        });
      }
      ...
      if (scope === 'all' || scope === 'standards') {
        (cachedData.entries && cachedData.entries.standards || []).forEach(function (entry) {
          items.push(entry);
        });
      }
      ...
      renderMixedList('environment-mixed-feed', filtered);
    }
