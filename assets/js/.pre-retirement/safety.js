(function () {
  ...
  function renderStats() {
    var container = document.getElementById('safety-stats');
    if (!container) return;
    fetch('assets/data/safety.json')
      ...
      .catch(function () {
        container.innerHTML = '<div class="activity-empty">Operational summary is temporarily unavailable.</div>';
      });
  }
  ...
  function renderPillars() {
    var container = document.getElementById('safety-pillars');
    if (!container) return;
    fetch('assets/data/safety.json')
      .then(function (res) { return res.json(); })
      .then(function (data) {
  ...
  function renderFrameworksFeed() {
    loadSection('safety-frameworks-feed', 'frameworks', 'Frameworks');
  }

  function renderIncidentsFeed() {
    loadSection('safety-incidents-feed', 'incidents', 'Incidents');
  }

  function renderProtocolsFeed() {
    loadSection('safety-protocols-feed', 'protocols', 'Protocols');
  }
  ...
  function buildExtraMeta(item) {
    var parts = [];
    if (item.severity) parts.push('<span class="meta-budget">Severity: ' + escapeHtml(String(item.severity)) + '</span>');
    if (item.reportedDate) parts.push('<span class="meta-date">Reported: ' + escapeHtml(String(item.reportedDate)) + '</span>');
    if (item.resolvedDate) parts.push('<span class="meta-date">Resolved: ' + escapeHtml(String(item.resolvedDate)) + '</span>');
    ...
    if (item.nodeCoverage) parts.push('<span class="metric">Coverage: ' + escapeHtml(String(item.nodeCoverage)) + '</span>');
    return parts.join('<span class="activity-divider">·</span>');
  }

  /* ---- filters ---- */
  function initFilters() {
    var searchField = document.getElementById('safety-search');
    var statusField = document.getElementById('safety-status');
    var typeField = document.getElementById('safety-type');
    ...
    function applyFilter() {
      ...
      ['frameworks', 'incidents', 'protocols'].forEach(function (key) {
      ...
      var filtered = allItems.filter(function (entry) {
        var searchable =
          ...
          (entry.summary || '') + ' ' +
          (entry.category || '') + ' ' +
          (entry.owner || entry.lead || '') + ' ' +
          (entry.status || '');
        var matchesTerm = !term || String(searchable).toLowerCase().indexOf(term) !== -1;
        var matchesStatus = status === 'all' || entry.status === status;
        var matchesType = type === 'all' || entry.category === type || (type === 'framework' && entry.category === 'Framework') || (type === 'protocol' && entry.category === 'Protocol') || (type === 'incident' && entry.category === 'Incident');
        return matchesTerm && matchesStatus && matchesType;
      });

      renderMixedList('safety-mixed-feed', filtered);
    }
