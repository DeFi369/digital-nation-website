(function () {
  ...
  function init() {
    initHealthEquityFeeds();
    initFilters();
  }

  function initHealthEquityFeeds() {
    renderFeed('health-equity-frameworks-feed', 'frameworks', 'Equity frameworks');
    renderFeed('health-equity-programmes-feed', 'programmes', 'Access programmes');
    renderFeed('health-equity-standards-feed', 'standards', 'Community health standards');
    renderFeed('health-equity-reports-feed', 'reports', 'Health equity reports');
  }

  function renderFeed(feedId, sectionKey, placeholderText) {
  ...
  function initFilters() {
    ...
    var scopeField = document.getElementById('health-equity-scope');
    ...
    function applyFilter() {
      ...
      var items = [];
      if (scope === 'all' || scope === 'frameworks') {
        (cachedData.entries && cachedData.entries.frameworks || []).forEach(function (entry) { items.push(entry); });
      }
      ...
      renderMixed('health-equity-mixed-feed', filtered);
    }
