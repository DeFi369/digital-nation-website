(function () {
  ...
  function renderStats() {
    ...
    fetch('assets/data/health.json')
      ...
      .catch(function () {
        container.innerHTML = '<div class="activity-empty">Health summary is temporarily unavailable.</div>';
      });
  }
  ...
  function renderPillars() {
    const container = document.getElementById('health-pillars');
    if (!container) return;
    fetch('assets/data/health.json')
      .then(function (res) { return res.json(); })
      .then(function (data) {
  ...
  function renderServices() {
    loadSection('health-services-feed', 'services', 'Digital health services');
  }

  function renderWellness() {
    loadSection('health-wellness-feed', 'wellness', 'Wellness programs');
  }

  function renderReports() {
    loadSection('health-reports-feed', 'reports', 'Health governance and audit reports');
  }
  ...
  function initFilters() {
    var searchField = document.getElementById('health-search');
    var scopeField = document.getElementById('health-scope');
    ...
    function applyFilter() {
      ...
      if (scope === 'all' || scope === 'services') {
        (cachedData.entries && cachedData.entries.services || []).forEach(function (entry) {
          items.push(entry);
        });
      }
      if (scope === 'all' || scope === 'wellness') {
        (cachedData.entries && cachedData.entries.wellness || []).forEach(function (entry) {
          items.push(entry);
        });
      }
      ...
      renderMixedList('health-mixed-feed', filtered);
    }
