(function () {
  ...
  function init() {
    try {
      ...
      var statusField = document.getElementById('transparency-status');
  ...
  function loadFeeds() {
    fetch('assets/data/transparency.json')
      .then(function (response) {
        if (!response.ok) throw new Error('transparency data unavailable');
        return response.json();
      })
      .then(function (data) {
        var entries = data.entries || {};
        renderSection('transparency-requests-feed', entries.requests || []);
        renderSection('transparency-disclosures-feed', entries.disclosures || []);
        renderSection('transparency-reports-feed', entries.reports || []);
      })
      .catch(function () {
        var ids = [
          'transparency-requests-feed',
          'transparency-disclosures-feed',
          'transparency-reports-feed'
        ];
        ids.forEach(function (id) {
          var el = document.getElementById(id);
          if (el) el.innerHTML = '<div class="activity-empty">Transparency data is temporarily unavailable.</div>';
        });
      });
  }
  ...
  function renderSection(containerId, items) {
    ...
    container.innerHTML = filtered
      .map(function (entry) {
        ...
        if (entry.owner) meta.push(escapeHtml(String(entry.owner)));
        ...
        var html =
          '<article class="activity-item transparency-item" role="article" aria-label="' +
          escapeHtml(String(entry.id || '')) +
          '">' +
        ...
        html +=
          ...
          '<span class="transparency-status transparency-status-' +
